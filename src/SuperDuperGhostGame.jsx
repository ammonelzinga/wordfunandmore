...
export default function SuperDuperGhostGame() {
  ...
  async function addLetter(letter, position, reverse) {
    ...
    if (reverse) {
      newFragment = newFragment.split("").reverse().join("");
    }
    if (position === "start") {
      newFragment = letter + newFragment;
    } else {
      newFragment = newFragment + letter;
    }
  ...
  async function botMove() {
    ...
    for (const l of alphabet) {
      ...
      if (await isValidFragment(l + fragment) && !(await isCompleteWord(l + fragment))) {
        options.push({ l, pos: "start", reverse: false });
      }
      if (await isValidFragment(fragment + l) && !(await isCompleteWord(fragment + l))) {
        options.push({ l, pos: "end", reverse: false });
      }
      // Reverse moves
      const reversedFragment = fragment.split("").reverse().join("");
      if (await isValidFragment(l + reversedFragment) && !(await isCompleteWord(l + reversedFragment))) {
        options.push({ l, pos: "start", reverse: true });
      }
      if (await isValidFragment(reversedFragment + l) && !(await isCompleteWord(reversedFragment + l))) {
        options.push({ l, pos: "end", reverse: true });
      }
    }
    if (options.length === 0) {
    ...
    const { l, pos, reverse } = options[Math.floor(Math.random() * options.length)];
    addLetter(l, pos, reverse);
  }
      ...
      <div style={{ 
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px" }}>
            ...
            <h2 style={{ margin: 0, color: "#2c3e50", fontSize: "2.5em" }}>SuperDuperGhost Setup</h2>
            <div></div>
          </div>
    ...
    <div style={{ 
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px" }}>
          ...
          <h2 style={{ margin: 0, color: "#2c3e50", fontSize: "2.5em" }}>SuperDuperGhost</h2>
          <div></div>
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ fontSize: "16px", color: "#0277bd", marginBottom: "10px" }}>
            <strong>SuperDuperGhost Rules:</strong> Add letters to beginning/end, optionally reverse fragment first
          </div>
          <div style={{ fontSize: "14px", color: "#0288d1" }}>
            Purple bordered inputs reverse the fragment before adding the letter
          </div>
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
            <label style={{ fontSize: "14px", fontWeight: "600", color: "#764ba2" }}>Reverse + End</label>
            <input
              type="text"
              maxLength={1}
              style={{
              ...
              disabled={message || challengeMode || gameWinner || !fragment || (mode === "bot" && turn % 2 === 1)}
              onKeyDown={e => {
                if (/^[a-zA-Z]$/.test(e.key)) {
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
            <label style={{ fontSize: "14px", fontWeight: "600", color: "#764ba2" }}>Reverse + Start</label>
            <input
              type="text"
              maxLength={1}
              style={{
